...
set my_current_file_name     opt4c_int_fp_mode_wrapper_pipepe_intclean_int
...
# INT-clean diagnostic. This report constrains the active INT mode and removes
# inactive FP sequential paths, including internal scheduler reg-to-reg paths
# that are not covered by only false-pathing FP ports.
set_case_analysis 0 [get_ports mode_fp]
set_false_path -from [get_ports fp_*]
set_false_path -to [get_ports fp_*]

set fp_scheduler_regs [get_cells -hierarchical -quiet -filter "is_sequential == true && full_name =~ fp_scheduler/*"]
if {[sizeof_collection $fp_scheduler_regs] > 0} {
    set_false_path -from $fp_scheduler_regs
    set_false_path -to $fp_scheduler_regs
}
