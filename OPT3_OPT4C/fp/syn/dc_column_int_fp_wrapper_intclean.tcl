...
set my_verilog_list          "filelist_column_int_fp_wrapper.f"
set my_current_design_name   opt4c_column_int_fp_mode_wrapper
set my_current_file_name     opt4c_column_int_fp_mode_wrapper_intclean_int
...
# INT-clean diagnostic for the N=4 column wrapper. Apply mode isolation before
# compile so inactive FP scheduler/accumulator paths do not consume optimization
# effort or appear as reg-to-reg constraints in the INT-mode report.
set_case_analysis 0 [get_ports mode_fp]
set_false_path -from [get_ports fp_*]
set_false_path -to [get_ports fp_*]

foreach fp_reg_pattern {
    current_*_reg*
    encoded_a_reg*
    bw_*_reg*
    drain_*_reg*
...
set_boundary_optimization [get_cells shared_column] false
set_ungroup [get_cells shared_column] false
