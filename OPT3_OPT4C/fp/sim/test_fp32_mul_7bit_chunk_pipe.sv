module test_fp32_mul_7bit_chunk_pipe;
...
initial begin
    ...
    check_against_ref(32'h3f800000, 32'h3f800000); // 1.0 * 1.0
    check_against_ref(32'h3fc00000, 32'h40000000); // 1.5 * 2.0
    check_against_ref(32'hbfc00000, 32'h40000000); // -1.5 * 2.0
    check_against_ref(32'h00000000, 32'h3f800000); // 0.0 * 1.0
    check_against_ref(32'h80000000, 32'h3f800000); // -0.0 * 1.0
    check_against_ref(32'h7f800000, 32'h40000000); // inf * 2.0
    check_against_ref(32'hff800000, 32'h40000000); // -inf * 2.0
    check_against_ref(32'h7f800000, 32'h00000000); // inf * 0.0
    check_against_ref(32'h7fc12345, 32'h3f800000); // qNaN * 1.0
    check_against_ref(32'h7f7fffff, 32'h40000000); // overflow

    repeat (2000) begin
        check_against_ref(random_normal(), random_normal());
    end

    $display("\033[1;32mSUCCESS: fp32 7-bit chunk pipelined multiply tests passed.\033[0m");
    $finish;
end
...
task check_against_ref;
    ...
    begin
        ...
        if (result !== expected_result ||
            ...
            $error("pipeline mismatch test=%0d a=%h b=%h expected=%h got=%h flags expected=%b%b%b%b got=%b%b%b%b",
