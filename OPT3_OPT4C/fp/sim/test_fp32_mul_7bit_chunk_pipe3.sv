module test_fp32_mul_7bit_chunk_pipe3;
...
initial begin
    ...
    $display("\033[1;32mSUCCESS: fp32 7-bit chunk 3-stage pipelined multiply tests passed.\033[0m");
    $finish;
end
...
task check_against_ref;
    ...
    begin
        ...
        wait_count = 0;
        while (!valid_out && wait_count < 8) begin
            @(posedge clk);
            #1;
        ...
        if (result !== expected_result ||
            ...
            $error("pipe3 mismatch test=%0d a=%h b=%h expected=%h got=%h flags expected=%b%b%b%b got=%b%b%b%b",
