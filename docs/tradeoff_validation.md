...
```text
top_pe_pipe_baseline @ 0.59 ns:
    slack = 0.01, area = 1335.526738

opt4c_int_fp_mode_wrapper_pipepe_int @ 0.59 ns:
    slack = -0.72, area = 17106.941188
```

The full-wrapper top paths are not in the PE anymore. They are auto-generated
adder/register paths such as `add_x_7/R_225 -> R_491`, which indicates the
reported violation is coming from FP-side arithmetic/accumulation registers
inside the dual-mode wrapper. This means the `MODE=int` report generated by
`dc_int_fp_wrapper_pipepe.tcl` is still polluted by inactive FP internal
register-to-register paths. To isolate the true INT active-mode timing, use
`dc_int_fp_wrapper_pipepe_intfirst.tcl`, which applies `set_case_analysis 0
